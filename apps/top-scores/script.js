...
const SPREADSHEET_ID = "1PJZ50b5Y6RqHNW1PH5BFn-wvc7IPxu3m0JastDksMtQ";
...
const SHEETS = [
  ...
  {
    tab: "Awaytris",
    title: "Awaytris Highscore (Traintris, Planetris, Cartris...)",
  },
...
const PLAYER_ALIASES = {
  ...
  "Blue Scuti": ["Blue_Scuti", "BlueScuti"],

  Creepercraftyt: ["Creepercraft"],
...
async function loadPlayerDirectory() {
  ...
  players.forEach((player) => {
    ...
    const parenthetical = String(player.name || "").match(/^(.+?)\s*\((.+)\)$/);
    if (parenthetical) {
...
function mappedPlayer(name) {
  return (
    ...
    PLAYER_DIRECTORY.get(normalize(name).replace(/[^a-z0-9]+/g, ""))
  );
}
...
async function loadLeaderboards() {
  ...
  if (copyFeedback) {
    copyFeedback.textContent = `${uniquePlayers.length} unique source name${uniquePlayers.length === 1 ? "" : "s"}`;
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    window[callbackName] = (data) => {
      ...
      try {
        ...
        if (metricColumn === -1 && !(isBType && col.lines !== -1)) {
          throw new Error(
        ...
        const rows = data.table.rows
          .map((row) => {
            ...
            const scoreCell = col.score !== -1 ? cells[col.score] : null;

            const linesCell = col.lines !== -1 ? cells[col.lines] : null;

            const metricCell = useLines ? linesCell : scoreCell;
          ...
          .filter((item) => item.name && Number.isFinite(item.metricValue));
        ...
        rows.sort((a, b) => {
          ...
          if (isBTypeCategory(sheetName)) {
            ...
            if (a.bTypeUsesLines && b.bTypeUsesLines) {
              return (
                b.linesValue - a.linesValue || a.name.localeCompare(b.name)
              );
            }

            return b.scoreValue - a.scoreValue || a.name.localeCompare(b.name);
          }
          ...
          // Everything else: highest metric first.
          const ascending =
            isTimeCategory(sheetName) || normalize(sheetName) === "earl max";
...
function renderMedalStandings(categories) {
      ...
      category.rows.slice(0, 3).forEach((row, index) => {
        ...
        if (!isDuplicate) {
          ...
          if (index === 1) {
            ...
            person.duplicateSilverMetrics.push(
              formatMedalMetric(row, category),
            );
          }
          ...
          if (index === 2) {
            ...
            person.duplicateBronzeMetrics.push(
              formatMedalMetric(row, category),
            );
          }
        }
  ...
  const standings = [...people.values()].sort(
    (a, b) =>
      b.gold - a.gold ||
      b.silver - a.silver ||
      b.bronze - a.bronze ||
  ...
  standings.forEach((person, index) => {
    const medalKey = `${person.gold}|${person.silver}|${person.bronze}`;
  ...
  if (medalButton) {
    medalButton.addEventListener("click", () => {
      const expanded = medalButton.getAttribute("aria-expanded") === "true";
      ...
      medalButton.setAttribute("aria-expanded", String(!expanded));

      medalButton.textContent = expanded ? "Show more" : "Show less";
    });
  }
...
function formatMedalMetric(row, category) {
  ...
  if (row.timing) {
    ...
    if (sheetName === "sr 30 l" || sheetName === "sr 40 l") {
      const frameCount = `${formatInteger(row.timing.totalFrames)} frames`;
      ...
      return `${frameCount} ` + `(Time ${row.timing.originalTime})`;
    }
...
function render(categories) {
            ...
            >
              ...
              <div class="table-scroll">
                <table>
                  ...
                  <tbody>
                    ${category.rows
                      .map((row, index) => renderRow(row, index, category))
                      .join("")}
                  </tbody>
                </table>
              </div>
          ...
          <section class="leaderboard-group" id="${group.id}">
            ...
            >
              <div class="leaderboard-categories">
                ${groupedCategories
                  .map(({ category, categoryIndex }, index) =>
  ...
  document.querySelectorAll(".leaderboard-group-toggle").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      button.setAttribute("aria-expanded", String(!expanded));
      panel.hidden = expanded;
      button.querySelector(".leaderboard-group-icon").textContent = expanded
        ? "+"
        : "−";
    });
  });
  ...
  document.querySelectorAll(".leaderboard-search").forEach((search) => {
    search.addEventListener("input", () => {
      ...
      card.querySelectorAll("tbody tr[data-rank]").forEach((row) => {
        const isMatch = !query || normalize(row.dataset.player).includes(query);
        const showAllButton = card.querySelector(".show-all");
        const showAll = showAllButton?.getAttribute("aria-expanded") === "true";
        row.hidden = query
          ? !isMatch
  ...
  document.querySelectorAll(".chart-button").forEach((button) => {
    button.addEventListener("click", () => {
      openRankingChart(categories[Number(button.dataset.category)], button);
    });
  });
...
function renderRankingsToc(categories) {
  ...
  function setCurrentTocEntry(currentLink) {
    ...
    tocEntries.forEach(({ link }, index) => {
      ...
      if (isCurrent) {
        ...
        if (
          document.getElementById("site-menu")?.classList.contains("active")
        ) {
  ...
  if (window.__rankingsTocScrollHandler) {
    window.removeEventListener("scroll", window.__rankingsTocScrollHandler);
  }
  ...
  window.addEventListener("scroll", window.__rankingsTocScrollHandler, {
    passive: true,
  });
  updateCurrentTocEntry();
}
...
function renderRow(row, index, category) {
  ...
  const rowInformation = row.notes?.trim();
...
function openRankingChart(category, trigger) {
  ...
  const minimum = positiveValues.length ? Math.min(...positiveValues) : 1;
  const ascending = isTimeCategory(category.sheetName || "");

  lastChartTrigger = trigger;
  title.textContent = category.category;
  subtitle.textContent = `Top ${rows.length} · ranked by ${category.metricName}`;
  plot.innerHTML = rows
    .map((row, index) => {
      ...
      if (Number.isFinite(value) && value > 0) {
        width = ascending ? (minimum / value) * 100 : (value / maximum) * 100;
      }
...
function linkifyNote(note) {
  ...
  return escaped
    .replace(/(https?:\/\/[^\s<]+)/gi, (url) => {
      const safe = safeUrl(url.replace(/[),.;!?]+$/, ""));
...
function parseRunTime(value, sheetName, dateText) {
  ...
  const { minutes, seconds, frameText, normalizedTime } = parsed;

  const is30Lines = sheet === "sr 30 l";
  const is40Lines = sheet === "sr 40 l";
  const is100Lines = sheet === "sr 100 l" || sheet === "sr 100 l lvl 0";
  ...
  if (is30Lines) {
    ...
    is2023 = year === 2023 || (year === null && /[A-F]/i.test(frameText));
  }
  ...
  if (is100Lines) {
    ...
    const totalFrames = decimalFrames + 60 * (seconds + 60 * minutes);
  ...
  const frames = parseInt(frameText || "0", is2023 ? 16 : 10);
...
function parseDisplayedRunTime(rawValue) {
  ...
  let match = raw.match(/^(\d+):(\d{1,2})(?:\.([0-9A-Fa-f]+))?$/);
...
function formatAmericanDate(value) {
  ...
  let month;
  let day;
  let match = text.match(/^Date\(\s*(\d{4})\s*,\s*(\d{1,2})\s*,\s*(\d{1,2})/);
...
function formatSecondaryMetric(row, category) {
  ...
  if (category.metricName === "Lines" && Number.isFinite(row.scoreValue)) {
  ...
  if (category.metricName === "Score" && Number.isFinite(row.linesValue)) {
    return `Lines: ${new Intl.NumberFormat("en-US", {
      maximumFractionDigits: 0,
    }).format(row.linesValue)}`;
  }
...
function getPlaystyleTag(styleText) {
  ...
  if (text === "thor" || text.includes("thor grip")) {
    return {
      label: "Thor Grip",
      className: "tag-thor",
    };
...
function getPlatformTag(platformText) {
  ...
  if (text.includes("color") || text.includes("colour") || text === "gbc") {
...
function findColumn(columns, aliases) {
  return columns.findIndex((column) => aliases.includes(column.label));
}
...
function getUniquePlayers() {
  return [...RAW_PLAYERS.values()]
    .map((rawName) => {
      ...
      const isAlias = normalize(rawName) !== normalize(canonical);

      return {
        rawName,
        canonical,
        isAlias,
        display: isAlias ? `${rawName} → ${canonical}` : rawName,
      };
    })
    .sort((a, b) =>
      a.rawName.localeCompare(b.rawName, undefined, { sensitivity: "base" }),
    );
}
async function copyUniquePlayersToClipboard() {
  ...
  const textToCopy = players.map((player) => player.display).join("\n");

  try {
    await navigator.clipboard.writeText(textToCopy);

    if (feedback) {
      feedback.textContent = `Copied ${players.length} unique source names.`;
    }
  } catch (error) {
