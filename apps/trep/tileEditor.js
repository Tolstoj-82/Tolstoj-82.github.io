...
let disabledImportTileIndexes = new Set();
...
function renderTileImportDialog(fileName) {
  ...
  disabledImportTileIndexes = new Set();
  importMarqueeAnchor = null;
  importMarqueeActive = false;
  importMarqueeOrigin = null;
  importSelectionAnchorIndex = null;

  const firstTileByPixels = new Map();
  const duplicateOf = new Map();
  tileImportState.tiles.forEach((pixels, index) => {
    const signature = pixels.join("");
    if (firstTileByPixels.has(signature)) {
      duplicateOf.set(index, firstTileByPixels.get(signature));
      disabledImportTileIndexes.add(index);
    } else {
      firstTileByPixels.set(signature, index);
    }
  });
  ...
  tileImportState.tiles.forEach((pixels, index) => {
    ...
    const duplicateSourceIndex = duplicateOf.get(index);
    const isDisabledDuplicate = duplicateSourceIndex !== undefined;
    tile.draggable = !isDisabledDuplicate;
    tile.dataset.importTileIndex = index;
    if (isDisabledDuplicate) {
      tile.classList.add("duplicate-disabled");
      tile.dataset.duplicateOf = duplicateSourceIndex;
      tile.setAttribute("aria-disabled", "true");
      tile.tabIndex = 0;
      tile.title = `Tile ${index + 1} duplicates tile ${duplicateSourceIndex + 1}. Click to activate it.`;
    } else {
    ...
    if (isDisabledDuplicate) {
      const duplicateToggle = document.createElement("button");
      duplicateToggle.className = "duplicate-tile-toggle";
      duplicateToggle.type = "button";
      duplicateToggle.draggable = false;
      duplicateToggle.setAttribute("aria-label", `Activate duplicate tile ${index + 1}`);
      duplicateToggle.title = `Activate duplicate tile ${index + 1}`;
      duplicateToggle.addEventListener("pointerdown", event => event.stopPropagation());
      duplicateToggle.addEventListener("click", event => {
        event.stopPropagation();
        toggleDuplicateImportTile(index);
      });
      tile.appendChild(duplicateToggle);
    }

    tile.addEventListener("dragstart", event => {
      if (disabledImportTileIndexes.has(index)) {
    ...
    tile.addEventListener("keydown", event => {
      if (
        disabledImportTileIndexes.has(index)
...
function initializeTileImportDialog() {
  ...
  grid.addEventListener("pointerdown", event => {
    ...
    if (event.target.closest(".duplicate-tile-toggle")) return;
...
function selectImportedTiles(indexes) {
  selectedImportTileIndexes = new Set(
    Array.from(indexes).filter(index => !disabledImportTileIndexes.has(index))
  );
...
function toggleDuplicateImportTile(index) {
  const tile = document.querySelector(
    `#tileImportGrid .imported-tile[data-import-tile-index="${index}"]`
  );
  const toggle = tile?.querySelector(".duplicate-tile-toggle");
  if (!tile || !toggle || tile.dataset.duplicateOf === undefined) return;

  if (disabledImportTileIndexes.has(index)) {
    disabledImportTileIndexes.delete(index);
    tile.classList.remove("duplicate-disabled");
    tile.classList.add("duplicate-activated");
    tile.draggable = true;
    tile.removeAttribute("aria-disabled");
    tile.removeAttribute("tabindex");
    tile.title = `Tile ${index + 1} (duplicate activated)`;
    toggle.setAttribute("aria-label", `Deactivate duplicate tile ${index + 1}`);
    toggle.title = `Deactivate duplicate tile ${index + 1}`;
    selectImportedTiles([index]);
  } else {
    disabledImportTileIndexes.add(index);
    tile.classList.add("duplicate-disabled");
    tile.classList.remove("duplicate-activated");
    tile.draggable = false;
    tile.setAttribute("aria-disabled", "true");
    tile.tabIndex = 0;
    tile.title = `Tile ${index + 1} duplicates tile ${Number(tile.dataset.duplicateOf) + 1}. Click the × to activate it.`;
    toggle.setAttribute("aria-label", `Activate duplicate tile ${index + 1}`);
    toggle.title = `Activate duplicate tile ${index + 1}`;
    const selectedIndexes = new Set(selectedImportTileIndexes);
    selectedIndexes.delete(index);
    selectImportedTiles(selectedIndexes);
  }
}
...
async function replaceImportedTiles(replacements, isEntireSet, wasTruncated = false) {
  if (replacements.length === 0) return;
  const overlay = document.getElementById("tilePlacementOverlay");
  if (overlay) {
    const title = overlay.querySelector("h2");
    if (title) title.textContent = "Placing Tiles...";
